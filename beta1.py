...
def handle_text(message):
    ...
    print("-------------------------------------------------------------------------------2222222222222")
    ll9 = ""
    prompt2 ="расскажи по подробнее про (" + ww_vars[15] + ww_vars[16] + ww_vars[17] + ") , ответ должен быть расписан очень подробно и расказывать всю суть , по надобности приводить примеры того или иного, ответ дай в стиле реферата ,то есть ответ должен представлять чать реферата "
    response = openai.ChatCompletion.create(
        model=model_engine,
        messages=[
            {"role": "system", "content": "You are a helpful assistant."},
    ...
    variables_to_write = [ll2, ll6q1, ll4, ll5, ll6, ll7, ll8, ll9]
